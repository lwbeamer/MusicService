...
- ~~Получить все альбомы(album_id) по artist_id ну либо по user_id, но тогда с проверкой что он артист.~~(сделал)
- ~~При добавлении трека в юхер плейлист проверять что его у него нет~~(сделал)
- ~~Убрать повторное создание плейлиста юзера~~(сделал)
- ~~Убери с бэка изначальную дата базу с плейлистами которую мы не юзаем~~(сделал)

- createSong не работает. всегда возвращает ошибку, но песню создаёт, но лишь с одним артистом (от которого реквест пришёл)
- в getArtistByUserId помимо orgId выдай ещё orgName
- Сделай так чтобы юзер никак не смог получить доступ до непроверенных треков (т.е. только админ мог получить)
